                             ...
                             const CHAR *Password/* =NULL */ )
{
    __ENTER_FUNCTION
        ...
        if ( ( SQL_SUCCESS != mResult ) && ( SQL_SUCCESS_WITH_INFO != mResult ) ) 
        {
            CHAR LogBuff[ 512 ] ;
...
BOOL ODBCInterface::Execute()
{
    _MY_TRY
    {
        ...
        if ( ( SQL_SUCCESS != mResult ) && ( SQL_SUCCESS_WITH_INFO != mResult ) && ( SQL_NO_DATA != mResult ) ) 
        {
            DiagState() ;
            return FALSE ;
        }
...
BOOL ODBCInterface::LongFetch()
{
    __ENTER_FUNCTION
        ...
        for ( INT ColIndex = 0; ColIndex < mColCount; ColIndex++ )
        {
            ...
            INT                Ret      = 0 ;
            while( SQL_NO_DATA != ( Ret = SQLGetData( hStmt, ColIndex+1, SQL_C_CHAR, Col[ ColIndex ] + TotalGet, MAX_LONG_COLUMN_BUFFER, &LenData ) ) )
            {
                break ;
            }
...
INT ODBCInterface::GetInt( INT ColIndex, INT& ErrorCode )
{
    __ENTER_FUNCTION
        ...
        if ( SQL_NULL_DATA == mCollocate[ ColIndex - 1 ] )
        {
            ErrorCode = QUERYNULL ;
            Assert( FALSE ) ;
            return QUERYNULL ;
...
VOID    ODBCInterface::DiagState()
{
    __ENTER_FUNCTION
        ...
        SQLSMALLINT        MsgLen ;
        memset( mErrorMsg, 0, MAX_ERROR_MSG_LENGHT ) ;    
        while ( SQL_NO_DATA != ( mResult = SQLGetDiagRec( SQL_HANDLE_DBC, hDbc, ii, SqlState, &NativeError, 
            mErrorMsg, sizeof( mErrorMsg ), &MsgLen ) ) ) 
        {
            ii++ ;
        ...
        {
            // 自定义数据库错误
            mResult = SQLError( hEnv, hDbc, hStmt, SqlState, &NativeError, mErrorMsg,
                sizeof( mErrorMsg ), &MsgLen ) ;
        }
...
VOID    ODBCInterface::DiagStateEx()
{
    __ENTER_FUNCTION
        ...
        if( 0 == strlen( (const CHAR* )mErrorMsg) )
        {
            //自定义数据库错误
            mResult = SQLError( hEnv, hDbc, hStmt, SqlState, &NativeError, mErrorMsg, 
                sizeof( mErrorMsg ), &MsgLen ) ;
