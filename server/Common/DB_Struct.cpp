...
//战斗信息
    ...
    //玩家商店
    //
    for ( UINT i = 0; MAX_SHOP_NUM_PER_PLAYER > i; i++ )
    {
        m_ShopGuid[ i ].Reset() ;
    }

    for ( UINT i = 0; MAX_FAVORITE_SHOPS > i; i++ )
    {
...
{
    UINT i ;
    for ( i = 0; HUMAN_PET_MAX_COUNT > i; i++ )
    {
        m_aPetDB[i].CleanUp() ;
    }
}
...
{
    INT i;
    for ( i = 0; MAX_CHAR_MISSION_NUM > i; i++ )
...
{
    INT i;
    for ( i = 0; MAX_RELATION_SIZE > i; i++ )
    {
        m_aRelation[i].CleanUp() ;
    }
    for( i = 0; RELATION_GROUP_NUMBER > i; i++ )
    {
