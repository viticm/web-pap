...
namespace Combat_Module
{
    ...
    namespace Skill_Module
    {
        ...
        BOOL SkillTemplateData_T::InitFromDB(DBCFile* pDB, INT const nRow)
        {
            ...
            m_nClassByUser = (SHORT)(pDB->Search_Posistion(nRow, column++)->iValue & 0xFFFF) ; //LM修改
            // Read Flag: is a passive skill
            m_nPassiveFlag = (SHORT)(pDB->Search_Posistion(nRow, column++)->iValue & 0xFFFF) ; //LM修改
            // Read Flag: mouse operation type
            m_nSelectType = (SHORT)(pDB->Search_Posistion(nRow, column++)->iValue & 0xFFFF) ; //LM修改
            // Read the operat-mode for pet skill;
            m_nOperateModeForPetSkill = (SHORT)(pDB->Search_Posistion(nRow, column++)->iValue & 0xFFFF) ; //LM修改
            // Read the rate of pet skill;
            m_nPetRateOfSkill = (SHORT)(pDB->Search_Posistion(nRow, column++)->iValue & 0xFFFF) ; //LM修改
            // Read the type of pet skill;
            m_nTypeOfPetSkill = (SHORT)(pDB->Search_Posistion(nRow, column++)->iValue & 0xFFFF) ; //LM修改
            // Read the ImpactID of pet skill;
            m_nImpactIDOfSkill = (SHORT)(pDB->Search_Posistion(nRow, column++)->iValue & 0xFFFF) ; //LM修改
            // Skip reversed fields and client data
            column += 7;
            // Read the Targeting logic id
            m_nTargetingLogic = (SHORT)(pDB->Search_Posistion(nRow, column++)->iValue & 0xFFFF) ; //LM修改
            ...
            m_nTargetLogicByStand = (SHORT)(pDB->Search_Posistion(nRow, column++)->iValue & 0xFFFF) ; //LM修改
            // Read the target Check by OBJ_TYPE
            m_nTargetCheckByObjType = (SHORT)(pDB->Search_Posistion(nRow, column++)->iValue & 0xFFFF) ; //LM修改
            ...
            INT k= pDB->GetFieldsNum();
            if(k>MAX_CHAR_SKILL_LEVEL) k=MAX_CHAR_SKILL_LEVEL;
            for(INT i=0;k>i;++i)
            {
        ...
        BOOL SkillInstanceData_T::InitFromDB(DBCFile* pDB, INT const nRow)
        {
            ...
            m_szDescDetail= pDB->Search_Posistion(nRow, 3)->pString;// LM修改
