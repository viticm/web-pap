...
INT DBCFile::_ConvertStringToVector( const CHAR* strStrINTgSource, 
                                     ...
                                     BOOL bIgnoreEmpty )
{
    __ENTER_FUNCTION
        ...
        while ( TRUE )
        {
            ...
            if ( 0 < strItem.length() || !bIgnoreEmpty )
            {
...
bool DBCFile::_FieldEqu( const FIELD& a, const FIELD& b )
{
    __ENTER_FUNCTION
        ...
        else
        {
            try
            {
                return ( 0 == strcmp( a.pString, b.pString ) );
            }
            catch(...)
            {
                return false;
...
const CHAR* DBCFile::_GetLineFromMemory( CHAR* pStringBuf, INT nBufSize, const CHAR* pMemory, const CHAR* pDeadEnd )
{
    ...
    __ENTER_FUNCTION
        ...
        if ( pMem >= pDeadEnd || 0 == *pMem ) return 0 ;
        ...
        *pStringBuf = 0 ;
        ...
        //skip all next \r and \n
        while ( pMem < pDeadEnd && 0 != *pMem && ( '\r' == *pMem || '\n' == *pMem ) ) pMem++ ;
...
BOOL DBCFile::OpenFromMemory( const CHAR* pMemory, const CHAR* pDeadEnd, const CHAR* szFileName )
{
    __ENTER_FUNCTION
        ...
        //----------------------------------------------------
        //判断是否是二进制格式
        if ( sizeof( FILE_HEAD ) <= pDeadEnd - pMemory && 0XDDBBCC00 == *( ( UINT* )pMemory ) )
        {
...
BOOL DBCFile::OpenFromMemoryImpl_Binary( const CHAR* pMemory, const CHAR* pDeadEnd, const CHAR* szFileName )
{
    __ENTER_FUNCTION
        ...
        if ( 0XDDBBCC00 != theHead.m_Identify )
        {
            return FALSE ;
        }
        //check memory size
...
BOOL DBCFile::OpenFromMemoryImpl_Text( const CHAR* pMemory, const CHAR* pDeadEnd, const CHAR* szFileName )
{
    __ENTER_FUNCTION
        ...
        {
            if ( "INT" == vRet[i] ) vFieldsType[i] = T_INT ;
            else if ( "FLOAT" == vRet[i] ) vFieldsType[i] = T_FLOAT ;
            else if ( "STRING" == vRet[i] ) vFieldsType[i] = T_STRING ;
            else
            {
                return FALSE ;
            }
                ...
                switch( vFieldsType[ i ] )
                {
                    ...
                    case T_STRING:
                        ...
                        else
                        {
                            ...
                            CHAR szSave[ 2000 ] = { 0 } ;
                            INT iRet ;
                            iRet = convert( "GBK", "UTF-8", szSave, 2000, (char*)p, strlen( p )  ) ;
                            LERR( "convert ret: %d, szSave: %s", iRet, szSave ) ;
        ...
        for ( register INT i = 0; i < nFieldsNum; i++ )
        {
            if ( T_STRING != vFieldsType[i] ) continue ;
...
VOID DBCFile::CreateIndex( INT nColum, const CHAR* szFileName )
{
    __ENTER_FUNCTION
        ...
        if ( 0 > nColum || nColum >= m_nFieldsNum || m_nIndexColum == nColum ) return ;
...
const DBCFile::FIELD* DBCFile::Search_Posistion( INT nRecordLine, INT nColumNum ) const
{
    __ENTER_FUNCTION
        ...
        if ( 0 > nPosition || nPosition >= ( INT )m_vDataBuf.size() ) 
        {
            CHAR szTemp[ 260 ] ;
...
{
    __ENTER_FUNCTION
        if ( 0 > nColumnNum || nColumnNum >= m_nFieldsNum ) return 0 ;
 ...
 */
INT DBCFile::convert(const char *from, const char *to, char* save, int savelen, char *src, int srclen)
{
    iconv_t cd;
    char   *inbuf = src;
    char *outbuf = save;
