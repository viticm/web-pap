...
VOID    ItemTable::InitGreenItemTable()
{
    ...
    for(INT i =0;i<iTableCount;i++)
    {
        ...
        m_pGreenEquipTableData[i].m_ReqLevel            =         (BYTE)(ThirdFile.Search_Posistion(i,TBReqLevel)->iValue & 0xFF);    //LM修改
        ...
        for(INT rate = 0; rate<IATTRIBUTE_NUMBER;rate++)
        {
            ...
            else
            {
                ...
                if(iValue>0 && iAttrCount<MAX_GREEN_ITEM_EXT_ATTR)
                {
                    ...
                    m_pGreenEquipTableData[i].m_Attr[iAttrCount].m_Value.m_Value        =    (BYTE)(iValue & 0xFF);    //LM修改
                    iAttrCount++;
                }
            }
...
VOID    ItemTable::InitCommItemInfoTable()
{
    ...
    for(UINT i = 0;i<m_nCommItemCount;i++)
    {
        ...
        m_pCommItemInfoData[i].m_nRulerID                        = (CHAR)(ThirdFile.Search_Posistion(i,TBRulerID)->iValue & 0xFF);        //LM修改
...
VOID    ItemTable::InitEquipSetAttrTable()
{
    ...
    for(UINT i =0;i<m_nEquipSetCount;i++)
    {
        ...
        INT k= (iTableColumn >IATTRIBUTE_NUMBER ) ? IATTRIBUTE_NUMBER : iTableColumn-1;    //LM修改
        for(INT j = 0;j<k;j++)
        {
            ...
            if((iValue>0)&&(iAttrCount<MAX_ITEM_SET_ATTR))
            {
                ...
                m_pEquipSetData[i].m_ItemAttr[iAttrCount].m_Value.m_Value        =    (SHORT)(iValue & 0xFFFF);     //LM修改
                iAttrCount++;
            }
