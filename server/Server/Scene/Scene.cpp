...
BOOL Scene::IsCanEnter()
{
    __ENTER_FUNCTION
        ...
        if ( FD_SETSIZE < iCount )
        {
            return FALSE ;
        }
        
        if ( SCENE_STATUS_RUNNING != GetSceneStatus() )
        {//不能尝试进入未开启的场景
            Assert( FALSE ) ;
            return FALSE ;
        }
...
INT Scene::CheckEnter( PlayerID_t PlayerID )
{
    __ENTER_FUNCTION
        ...
        if ( SCENE_TYPE_CIT == GetSceneType() ) // 如果此场景是副本场景
        {
            if ( m_SceneInitData.m_CityData.m_Guid.isNull() || SCENE_STATUS_RUNNING != GetSceneStatus() )
            {//场景还未被分配
                return -1 ;
            }
        }
...
BOOL Scene::ObjectEnterScene( Obj *pObj )
{
    __ENTER_FUNCTION
        ...
        if ( NULL == pObj )
        {
            Assert( NULL != pObj && "Scene::ObjectEnterScene" ) ;
            return FALSE ;
        }
...
VOID Scene::ObjectLeaveScene( Obj *pObj )
{
    __ENTER_FUNCTION
        ...
        if ( NULL == pObj )
        {
            Assert( NULL != pObj && "Scene::ObjectLeaveScene" ) ;
            return ;
        }
...
ZoneID_t Scene::CalcZoneID( const WORLD_POS* pos )
{
    __ENTER_FUNCTION
        ...
        if ( NULL == pos  || 0.f > pos->m_fX || 0.f >  pos->m_fZ )
        {
            Assert( ( NULL != pos &&  0.f <= pos->m_fX &&  0.f <= pos->m_fZ ) && "Scene::CalcZoneID" ) ;
            return INVALID_ID ;
        }
...
BOOL Scene::ObjZone_Register( Obj *pObj, ZoneID_t idZone )
{
    __ENTER_FUNCTION
        ...
        {
            Assert(  0 <= idZone && idZone < m_ZoneInfo.m_wZoneSize && "Scene::ObjZone_Register" ) ;
            return FALSE ;
        }
        ...
        {
            Assert( NULL == pPacket && "Scene::ObjZone_Register CreateNewObjPacket return NULL" ) ;
            return FALSE ;
        }
...
BOOL Scene::ObjZone_Unregister( Obj *pObj, ZoneID_t idZone )
{
    __ENTER_FUNCTION
        ...
        if ( NULL == pObj )
        {
            Assert( NULL == pObj && "Scene::ObjZone_Unregister" ) ;
            return FALSE ;
        }

        if ( 0 > idZone || idZone >= m_ZoneInfo.m_wZoneSize )
        {
            Assert( 0 <= idZone && idZone < m_ZoneInfo.m_wZoneSize && "Scene::ObjZone_Unregister" ) ;
            return FALSE ;
        }
        ...
        {
            Assert( NULL == pPacket && "Scene::ObjZone_Unregister CreateDeleteObjPacket return NULL" ) ;
            return FALSE ;
        }
...
BOOL Scene::ObjZone_Changed( Obj *pObj, ZoneID_t idNew, ZoneID_t idOld )
{
    __ENTER_FUNCTION
        if( NULL == pObj )
        {
            Assert( NULL == pObj && "Scene::ObjZone_Changed" ) ;
            return FALSE ;
        }

        if ( ( 0 > idNew || idNew >= m_ZoneInfo.m_wZoneSize ) && ( idOld < 0 || idOld >= m_ZoneInfo.m_wZoneSize ) )
        {
            Assert( ( 0 > idNew || idNew >= m_ZoneInfo.m_wZoneSize ) && 
                ( 0 > idOld || idOld >= m_ZoneInfo.m_wZoneSize ) && "Scene::ObjZone_Changed" ) ;
            return FALSE ;
        }

        if ( 0 > idOld || idOld >= m_ZoneInfo.m_wZoneSize )
        {
            Assert( 0 <= idOld && idOld < m_ZoneInfo.m_wZoneSize && "Scene::ObjZone_Changed" ) ;
        }
        else
        {
            ...
            {
                Assert( NULL == pPacket && "Scene::ObjZone_Changed 刷新Obj删除消息到客户端 CreateNewObjPacket return NULL" ) ;
                return FALSE ;
            }
            ...
            if ( Obj::OBJ_TYPE_HUMAN == pObj->GetObjType() )
            {
                ...
                for ( i = 0 ; i < listObj.m_Count ; i++ )
                {
                    ...
                    if ( pFindObj->IsCanViewMe( pHuman ) )
                    {
                        ...
                        {
                            Assert( NULL == pPacket && 
                                "Scene::ObjZone_Changed 把超出视口的角色删除消息发到客户端 CreateDeleteObjPacket return NULL" ) ;
                            return FALSE ;
                        }
                    }
                }
        ...
        if ( 0 > idNew || idNew >= m_ZoneInfo.m_wZoneSize )
        {
            Assert( 0 <= idOld && idOld < m_ZoneInfo.m_wZoneSize && "Scene::ObjZone_Changed" ) ;
        }
        else
        {
            ...
            if ( NULL != pPacket )
            {
                ...
                if ( 0 < listHuman.m_Count )
                {
                    SendPacket( pPacket, &listHuman ) ;
                    // 当发现玩家时才开始泡泡说话
                    if ( Obj::OBJ_TYPE_MONSTER == pObj->GetObjType() )
                    {
            ...
            {
                Assert( NULL == pPacket && "Scene::ObjZone_Changed 刷新Obj创建消息到客户端 CreateNewObjPacket return NULL" ) ;
                return FALSE ;
            }
            ...
            if ( Obj::OBJ_TYPE_HUMAN == pObj->GetObjType() )
            {
                ...
                for ( i = 0 ; i < listObj.m_Count ; i++ )
                {
                    ...
                    if ( pFindObj->IsCanViewMe( pHuman ) )
                    {
                        ...
                        {
                            Assert( NULL == pPacket && 
                                "Scene::ObjZone_Changed 把新进入的区域中的所有角色发往新进来的客户端 CreateDeleteObjPacket return NULL" ) ;
                            return FALSE ;
                        }
                    }
                }
...
VOID Scene::DeleteObject( Obj *pObj )
{
    __ENTER_FUNCTION
        ...
        if ( NULL == pObj )
        {
            Assert( NULL == pObj && "Scene::DeleteObject" ) ;
            return ;
        }
...
//这个函数没完成
CHAR* Scene::FormatMissionString( const CHAR* strIn, 
                                  CHAR* strOut, 
                                  const Obj_Human* pHuman, 
                                  const _MISSION_LIST_t* pMis, 
                                  const _MISSION_DATA_t* pData, 
                                  const _MISSION_REWARD_t* pReward, 
                                  BOOL& bConvert )
{
    __ENTER_FUNCTION
        //return strIn ;
        ...
        if ( NULL == strIn || NULL == pHuman ) return NULL ;
        if ( NULL == pData || NULL == pReward ) return NULL ;

        nLen = ( INT )strlen( strIn ) ;
        if ( 1024 < nLen ) return NULL ;
