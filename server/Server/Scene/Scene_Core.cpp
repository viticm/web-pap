...
BOOL Scene::HeartBeat()
{
    __ENTER_FUNCTION
        ...
        if ( m_QuitTimer.IsSetTimer() )
        {
            ...
            {
                g_pLog->FastSaveLog( LOG_FILE_1, "Scene %d has been closed!", SceneID() ) ;
        ...
        else
        {
            ...
            else if ( SCENE_TYPE_COPY == GetSceneType() )
            {//如果当前场景里面没有人，并且没有人
            ...
            else if ( SCENE_TYPE_CIT == GetSceneType() && m_CityData.m_bClose )
            {//启动关闭城市逻辑
                if ( !m_CityData.m_DynamicSceneQuitTimer.IsSetTimer() )
                {
                    m_CityData.m_DynamicSceneQuitTimer.BeginTimer( 5000, uTime ) ;
                    g_pLog->FastSaveLog( LOG_FILE_1, "Scene %d will been closed in %d seconds!", SceneID(), 5 ) ;
                }
...
BOOL Scene::ProcessCacheCommands()
{
    __ENTER_FUNCTION
        ...
        for ( UINT i = 0 ; i < m_QueSize ; i++ )
        {
            ...
            else if ( INVALID_ID_EX == PlayerID )
            {
                UINT uret ;
                _MY_TRY
                {
...
BOOL Scene::CloseScene()
{
    __ENTER_FUNCTION
        ...
        if ( !m_QuitTimer.IsSetTimer() && SCENE_STATUS_SLEEP != GetSceneStatus()
            && SCENE_TYPE_GAMELOGIC != GetSceneType() )
        {
            UINT uTime = 5000 ;
