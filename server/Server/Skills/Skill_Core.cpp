...
namespace Combat_Module
{
    namespace Skill_Module
    {
        ...
        //class SkillCore_T
        BOOL SkillCore_T::ProcessSkillRequest( Obj_Character& rMe, 
                                               SkillID_t nSkillID, 
                                               BYTE nLevel, 
                                               ObjID_t nTargetID, 
                                               ...
                                               GUID_t guidTarget ) const
        {
            __ENTER_FUNCTION
                ...
                if ( INVALID_ID == pSkillTemplate->GetSkillClass() &&
                    INVALID_ID == pSkillTemplate->GetMenPai() )
                {
                    nLevel = 1 ;
                }
        ...
        BOOL SkillCore_T::InstanceSkill( SkillInfo_T& rSkillInfoOut, Obj_Character& rMe, SkillID_t nSkill, BYTE nLevel ) const
        {
            __ENTER_FUNCTION
                ...
                //保存相关的心法等级
                if ( 0 == nLevel || MAX_CHAR_SKILL_LEVEL < nLevel )
                {
                    GUID_t nGUID = 0 ;
                    if ( Obj::OBJ_TYPE_HUMAN == rMe.GetObjType() )
                    {
