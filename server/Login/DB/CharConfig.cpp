...
VOID CharConfig::InitDefaultCharData()
{
    __ENTER_FUNCTION
        ...
        for ( INT i = 0; i < camp_Num; i++ )
        {
            ...
            m_StartScene[i].nSceneNum = f.ReadInt( "startscene", szTemp ) ;
            sprintf( szTemp, "pos_x%d", i ) ;
            f.ReadText( "startscene", szTemp, szTmp, 32 ) ;
            m_StartScene[i].fStartX = ( FLOAT )( atof( szTmp ) ) ;
            sprintf( szTemp, "pos_z%d", i ) ;
            f.ReadText( "startscene", szTemp, szTmp, 32 ) ;
        ...
        //读入心情数据
        strncpy( m_pUserData->m_Relation.m_szMood, "前世今生，今生前世！", MOOD_DATA_SIZE - 1 ) ;
...
VOID CharConfig::InitSkillData()
{
    ...
    for ( INT i = 0; i < iTableCount; i ++ )
    {
        ...
        INT iClass  = ThirdFile.Search_Posistion( i, 15 )->iValue ;
        if ( 0 == iClass )
        {
            if ( MATTRIBUTE_SHAOLIN == iMenPai ||
                 MATTRIBUTE_WUDANG == iMenPai ||
...
VOID CharConfig::InitLevel1Attr()
{
    __ENTER_FUNCTION
        ...
        Assert( MATTRIBUTE_NUMBER*CATTR_LEVEL1_NUMBER + 1 == iColumnCount ) ;
...
BOOL CharConfig::InitCharAttr( FULLUSERDATA* pData )    
{
    __ENTER_FUNCTION    
        ...
        //初始化阵营数据
        if ( CAMP1_PLAYER == pData->m_Human.m_CampData.m_nCampID )
        {
