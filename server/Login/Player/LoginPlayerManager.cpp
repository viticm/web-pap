...
BOOL LoginPlayerManager::Init()
{
    __ENTER_FUNCTION
        ...
        Assert( INVALID_SOCKET != m_SocketID ) ;
...
BOOL LoginPlayerManager::ProcessInputs()
{
    __ENTER_FUNCTION
        ...
        {
            for ( INT i = 0 ; ACCEPT_ONESTEP > i ; i++ )
            {
                if ( !AcceptNewConnection() ) break ;
            }
        }
...
BOOL LoginPlayerManager::DelPlayer( PlayerID_t pid )
{
    __ENTER_FUNCTION
        ...
        if ( INVALID_SOCKET == fd ) Assert( FALSE ) ;
    ...
    __ENTER_FUNCTION
        ...
        for ( UINT i = 0 ; i < nPlayerCount ; i++ )
        {
            ...
            else
            {
                ...
                if ( PS_LOGIN_CONNECT == pLoginPlayer->GetPlayerStatus() )
                {
                    if ( MAX_LOGIN_PLAYER_AUTH_TIME < uTime-pLoginPlayer->m_ConnectTime )
                    {
                        ret = RemovePlayer(pPlayer) ;
                        Assert( ret ) ;
                    }
