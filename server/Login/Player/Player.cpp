...
BOOL Player::ProcessCommand( BOOL Option )
{
    __ENTER_FUNCTION
        ...
        _MY_TRY
        {
            ...
            for( ; ; )
            {
                ...
                if ( ( PacketID_t )PACKET_MAX <= packetID )
                {//无效的消息类型
                    Assert( FALSE ) ;
                    return FALSE ;
                }
    ...
    //                    m_pSocketInputStream->Skip( PACKET_HEADER_SIZE+packetSize ) ;
                    ...
                    _MY_TRY
                    {
                        ...
                        {//出现异常错误，断开此玩家连接
                            if ( pPacket ) g_pPacketFactoryManager->RemovePacket( pPacket ) ;
                            return FALSE ;
                        }
                    ...
                    if ( pPacket && bNeedRemove ) g_pPacketFactoryManager->RemovePacket( pPacket ) ;
                }
                _MY_CATCH
                {
                    return FALSE ;
...
BOOL Player::SendPacket( Packet* pPacket )
{
    __ENTER_FUNCTION
        ...
        if ( NULL != m_pSocketOutputStream )
        {
            pPacket->SetPacketIndex( m_PacketIndex++ ) ;
            PacketID_t packetID = pPacket->GetPacketID() ;
            UINT w ;
            if ( PACKET_LC_MAXCOUNT > packetID )
            {
