...
UINT    CLAskCreateCharHandler::Execute(CLAskCreateChar* pPacket, Player* pPlayer)
{
    ...
    else if(g_pDBThreadManager->IsPoolTID(CurrentThreadID))
    {
        ...
        else
        {
            ...
            if( CharNumber > 0 )
                PlayerCampType = CAMP1_PLAYER;//pLoginPlayer->GetPlayerCamp(); //如果建立过角色，强制用上一个角色的阵营ID//LM修改
            else
            {
                PlayerCampType = CAMP1_PLAYER;//pPacket->GetCharCamp();//LM修改
            ...
            pPacket->SetMenPai(MATTRIBUTE_WUDANG);//LM修改
            ...
            //CreateCharOp.SetCampID(PlayerCampType);//LM修改
